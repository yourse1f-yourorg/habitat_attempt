...
if echo "${PATH_HABITAT_BINARY}" | grep -E "^.{2,}$" > /dev/null 2>&1 ; then
  if [ ! -f ${PATH_HABITAT_BINARY}/hab ]; then
    echo "
...
if ${SANE} = "true"; then
    ...
    echo "--> Send the binary executable of Habitat, to host '${REMOTE_HOST}' . . . ";
    scp ${PATH_HABITAT_BINARY}/hab ${REMOTE_HOST}:/home/${REMOTE_HOST_USER}/;
    
    echo "--> Creates the Habitat user: '${REMOTE_HAB_USER}' . . .";
    ssh -t ${REMOTE_HOST} ./${PREP_SCRIPT} ${REMOTE_HAB_USER} ${REMOTE_HABITAT_PASSWD};
    
    echo "--> Back from remote procedure to create Habitat user '${REMOTE_HAB_USER}'.";
    
else

  echo "\n  *** Typical execution looks like this : ";
  echo "export REMOTE_HOST_USER='you'; # a user on the machine where Habitat is to run";
  echo "export PATH_HABITAT_BINARY='/usr/bin/'; # a path on this machine that contains 'hab'";
  ...
  echo "export ./CommissionHabitatHost.sh";
